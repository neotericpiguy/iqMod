...
fm = @(t) fdev / 2 * (sawtooth(2 * pi * saw_rate * t) + 1);
#fm = @(t) fdev + 0 * t;
I = @(t) dBmToVpk(P_dBm + 20) * cos(2 * pi * fm(t).*t);
Q = @(t) dBmToVpk(P_dBm + 20) * cos(2 * pi * fm(t).*t + pi / 2);
...
fm = @(t) (fdev/saw_rate)* sin(2 * pi * saw_rate * t);
R = @(t) dBmToVpk(P_dBm) * cos(2 * pi * fc.*t + fm(t));
